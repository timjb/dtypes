...
instance (DFunctor r, DFunctor s) => DFunctor (FProd r s) where
  ffmap f (FProd x y) = FProd (ffmap f x) (ffmap f y)

instance (DApplicative r, DApplicative s) => DApplicative (FProd r s) where
  fpure x = FProd (fpure x) (fpure x)
  FProd x1 x2 <<*>> FProd y1 y2 = FProd (x1 <<*>> y1) (x2 <<*>> y2)

instance (DTraversable r, DTraversable s) => DTraversable (FProd r s) where
...
instance (DFunctor r, DFunctor s) => DFunctor (FSum r s) where
  ffmap f (FSum1 x) = FSum1 (ffmap f x)
  ffmap f (FSum2 y) = FSum2 (ffmap f y)

instance (DTraversable r, DTraversable s) => DTraversable (FSum r s) where
